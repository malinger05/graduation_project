"""
HTTP client for admin panel → middleware (https://mw.local, mTLS).

Uses admin-staff-client.pem (not the kiosk cert).
Requires: ./scripts/gen_admin_client_cert.sh
"""
...
def _default_client_cert() -> Path:
    return _tls_dir() / "admin-staff-client.pem"


def _default_client_key() -> Path:
    return _tls_dir() / "admin-staff-client.key"
...
def client_cert_path() -> Path:
    raw = os.environ.get("MTLS_ADMIN_CERT", "").strip()
    return Path(raw) if raw else _default_client_cert()


def client_key_path() -> Path:
    raw = os.environ.get("MTLS_ADMIN_KEY", "").strip()
...
def mtls_enabled(base_url: str | None = None) -> bool:
    if os.environ.get("MTLS_DISABLE", "").lower() in ("1", "true", "yes"):
        return False
    url = (base_url or os.environ.get("MIDDLEWARE_DIRECT_URL", "https://mw.local")).strip()
...
def request_kwargs(
    ...
    if not mtls_enabled(base_url):
        ...
        verify_url = base_url or os.environ.get("MIDDLEWARE_DIRECT_URL", "https://mw.local")
    ...
    if not cert.is_file() or not key.is_file():
        raise RuntimeError(
            "Admin mTLS client cert missing. Run: ./scripts/gen_admin_client_cert.sh\n"
